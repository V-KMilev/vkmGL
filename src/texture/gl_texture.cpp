...
namespace Core {
    namespace {
        ...
        GLenum minFilterToGL(TextureMinFilter f) {
            switch (f) {
        ...
        GLenum magFilterToGL(TextureMagFilter f) {
            switch (f) {
    ...
    void Texture2D::setData(const void* data, uint32_t width, uint32_t height, GLenum format, GLenum type) {
        ...
        VKM_GL_CHECK(glTexImage2D(
            GL_TEXTURE_2D,
            0,
            (format == GL_SRGB || format == GL_SRGB_ALPHA) ? GL_SRGB8_ALPHA8 : GL_RGBA8,
    ...
    void Texture2D::setWrap(TextureWrap s, TextureWrap t) {
        ...
        m_params.wrapT = t;
        bind();
        VKM_GL_CHECK(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, wrapToGL(m_params.wrapS)));
        VKM_GL_CHECK(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, wrapToGL(m_params.wrapT)));
        unbind();
    }
    ...
    void Texture2D::setFilter(TextureMinFilter minFilter, TextureMagFilter magFilter) {
        ...
        bind();
        VKM_GL_CHECK(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, minFilterToGL(m_params.minFilter)));
        VKM_GL_CHECK(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, magFilterToGL(m_params.magFilter)));
        unbind();
    }
    ...
    bool Texture2D::loadFromFile(const std::string& filePath, bool flipVertically) {
        ...
        GLint internalFormat = inferInternalFormat(channels, /*srgb*/ false);

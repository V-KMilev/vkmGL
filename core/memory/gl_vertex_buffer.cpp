...
namespace Core {
	VertexBuffer::VertexBuffer(const void* data, unsigned int size) : _mID(0), _mSize(size) {
		...
		VKM_GL_CHECK(glBufferData(GL_ARRAY_BUFFER, _mSize, data, GL_STATIC_DRAW));
	}
	...
	VertexBuffer::VertexBuffer(const VertexBuffer& other) : _mID(0), _mSize(0) {
		...
		// Create a new buffer
		VKM_GL_CHECK(glBufferData(GL_ARRAY_BUFFER, _mSize, nullptr, GL_STATIC_DRAW));
	...
	void VertexBuffer::unbind() const {
		VKM_GL_CHECK(glBindBuffer(GL_ARRAY_BUFFER, 0));
	}
